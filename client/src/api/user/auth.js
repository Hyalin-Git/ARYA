...
export async function getUserId() {
	try {
		const response = await axios({
			method: "GET",
			url: "http://arya-4pmj.onrender.com/login/success",
...
export async function getRefreshToken(token, uid) {
	try {
		const response = await axios({
			method: "GET",
			url: `http://arya-4pmj.onrender.com/api/auth/refresh-token?userId=${uid}`,
			withCredentials: true,
		});
...
export async function updateSession(token, uid) {
	try {
		const response = await axios({
			method: "POST",
			url: "http://arya-4pmj.onrender.com/api/auth/refresh-token",
			withCredentials: true,
			data: {
				refreshToken: token,
				userId: uid,
